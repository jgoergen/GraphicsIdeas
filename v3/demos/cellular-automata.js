const values = {
    ...
    CELLS_X: {
        Min: (v) => Math.floor(CANVAS_WIDTH / v.CELL_SIZE.Value),
        Max: (v) => Math.floor(CANVAS_WIDTH / v.CELL_SIZE.Value),
        TransitionFrames: 1,
        Ceiled: true
    },
    CELLS_Y: {
        Min: (v) => Math.floor(CANVAS_HEIGHT / v.CELL_SIZE.Value),
        Max: (v) => Math.floor(CANVAS_HEIGHT / v.CELL_SIZE.Value),
        TransitionFrames: 1,
        Ceiled: true
    },
    CELL_FADE: {
...
const checkCell = (_x, _y, p) => {
    ...
    */
    ...
    if (newState == 1) {
        p.ctx.fillRect(
            _x * p.values.CELL_SIZE.Value,
            _y * p.values.CELL_SIZE.Value,
