var ImageUtilities = {
    ...
    makeColorTransparent: function (_image, _color) {
        ...
        return ImageUtilities.replaceImageColor(_image, _color, { r: 0, g: 0, b: 0 });
    },
    ...
    superFastBlur: function (imageData, width, height, radius, iterations) {
        ...
        var mul_sum = ImageUtilities.sfb_mul_table[radius];
        var shg_sum = ImageUtilities.sfb_shg_table[radius];
    ...
    stackBoxBlur: function (imageData, width, height, radius, iterations) {
        ...
        var stackStart = new ImageUtilities.BlurStack();

        var stack = stackStart;
        for (i = 1; i < div; i++) {
            stack = stack.next = new ImageUtilities.BlurStack();
            if (i == radiusPlus1) var stackEnd = stack;
        }
        stack.next = stackStart;
        var stackIn = null;

        var mul_sum = ImageUtilities.sbb_mul_table[radius];
        var shg_sum = ImageUtilities.sbb_shg_table[radius];
        while (iterations-- > 0) {
            yw = yi = 0;
            for (y = height; --y > -1;) {
